#!/bin/bash
# --------------------------------------------------------------------
# Init USB gadget according to config (setup.env)
# --------------------------------------------------------------------
...
#	echo "deadbeefdeadbe11" > strings/0x409/serialnumber
	# set manufacturer
	echo "Club404" > strings/0x409/manufacturer
	# set product
	...
	if $USE_HID; then
		...
		cat $OPSEC_DIR/conf/report_desc > functions/hid.g1/report_desc
	fi
	...
	if $USE_RAWHID; then
		...
		cat $OPSEC_DIR/conf/raw_report_desc > functions/hid.g2/report_desc
	fi
	...
	if $USE_HID_MOUSE; then
		...
		cat $OPSEC_DIR/conf/mouse_combined_desc > functions/hid.g3/report_desc
	fi
	...
	if $USE_UMS; then
		...
		echo $OPSEC_DIR/USB_STORAGE/image.bin > functions/mass_storage.usb0/lun.0/file
	fi
